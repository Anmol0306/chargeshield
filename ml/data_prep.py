...
CRITICAL
  ...
  - usecols on read: read exactly the columns any model needs and no more.
    The 339 V-columns ARE loaded (the gradient-boosted model uses them) with a
    float32 dtype hint, because parsing them as float64 first is a ~1.6GB
    transient we would immediately downcast away.
  ...
  - Boundaries use < on the lower split and >= on the upper — never <= on
    ...
    just trusted.
  - Any transform that LEARNS (encoders, scalers, imputers) is fit on train
    only. Nothing in this file learns anything.
"""
